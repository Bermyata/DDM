...
namespace DrawingDataManager
{
    public class ExcelReader
    {
        public static List<Drawing> ReadData(String currentDirectory, Logger logger, out Boolean hasError)
        {
            ...
            Settings.InitializeNames(currentDirectory + Path.DirectorySeparatorChar + Settings.FileBlockName);
                ...
                foreach (ExcelWorksheet worksheet in wss)
                {
                    try
                    {
                        ...
                        for (var row = 2; row <= worksheet.Dimension.Rows; row++)
                        {
                            ...
                            String blockNameId = blockNameTemplate + cells[row, 2].GetValue<String>();
        ...
        private static Dictionary<String, Int32> GetHeaders(ExcelWorksheet ws)
        {
            ...
            for (var i = 2; i <= ws.Dimension.Columns; i++) result.Add(ws.Cells[1, i].GetValue<String>(), i);
